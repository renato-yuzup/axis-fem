...
/**
 * Collects nodal stress data, in a node-by-node basis.
 *
 * @sa NodeSetCollector
 */
class AXISCOMMONLIBRARY_API NodeStressCollector : public NodeSetCollector
{
public:
  /**
   * Creates a new node stress collector.
   *
   * @param targetSetName Node set name in which this collector will act.
   *
   * @return A new collector.
   */
  static NodeStressCollector& Create(const axis::String& targetSetName);
  ...
  virtual ~NodeStressCollector(void);
...
private:
  NodeStressCollector(const axis::String& targetSetName, const axis::String& customFieldName);
  NodeStressCollector(const axis::String& targetSetName, const axis::String& customFieldName,
