...
/**
 * Collects nodal reaction force data, in a node-by-node basis.
 *
 * @sa NodeSetCollector
 */
class AXISCOMMONLIBRARY_API NodeReactionForceCollector : public NodeSetCollector
{
public:
  /**
   * Creates a new node reaction force collector.
   *
   * @param targetSetName Node set name in which this collector will act.
   *
   * @return A new collector.
   */
  static NodeReactionForceCollector& Create(const axis::String& targetSetName);
   ...
   *
   * @param targetSetName Node set name in which this collector will act.
   * @param xState        Tells if X-component reaction force should be collected.
   * @param yState        Tells if Y-component reaction force should be collected.
   * @param zState        Tells if Z-component reaction force should be collected.
   *
   * @return A new collector.
   */
  static NodeReactionForceCollector& Create(const axis::String& targetSetName,
  ...
  virtual ~NodeReactionForceCollector(void);
...
private:
  NodeReactionForceCollector(const axis::String& targetSetName, const axis::String& customFieldName);
  NodeReactionForceCollector(const axis::String& targetSetName, const axis::String& customFieldName,
