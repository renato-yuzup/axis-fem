...
/**
 * Collects nodal velocity data, in a node-by-node basis.
 *
 * @sa NodeSetCollector
 */
class AXISCOMMONLIBRARY_API NodeVelocityCollector : public NodeSetCollector
{
public:
  /**
   * Creates a new node velocity collector.
   *
   * @param targetSetName Node set name in which this collector will act.
   *
   * @return A new collector.
   */
  static NodeVelocityCollector& Create(const axis::String& targetSetName);
   ...
   *
   * @param targetSetName Node set name in which this collector will act.
   * @param xState        Tells if X-component velocity should be collected.
   * @param yState        Tells if Y-component velocity should be collected.
   * @param zState        Tells if Z-component velocity should be collected.
   *
   * @return A new collector.
   */
  static NodeVelocityCollector& Create(const axis::String& targetSetName,
  ...
  virtual ~NodeVelocityCollector(void);
...
private:
  NodeVelocityCollector(const axis::String& targetSetName, const axis::String& customFieldName);
  NodeVelocityCollector(const axis::String& targetSetName, const axis::String& customFieldName,
