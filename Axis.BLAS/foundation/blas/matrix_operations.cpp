...
namespace {
  ...
  inline bool is_equalf(real x1, real x2)
  {
    real diff = MATH_ABS(x1*x1 - x2*x2);
    diff *= diff;
    return sqrt(diff) <= tolerance;
  }

  inline int count_equal(const real x[], int valueCount)
  {
    int n = 0;
...
AXISBLAS_API void afb::SymmetricEigen( real *eigenValues,  
  DenseMatrix *eigenProjections[], int& eigenCount, const SymmetricMatrix& m )
{
  ...
  */
  ...
  real Q = MATH_ABS((i1*i1 - 3*i2) / 9.0);
...
AXISBLAS_API void afb::SymmetricLogarithm( SymmetricMatrix& lhr, 
...
AXISBLAS_API void afb::SymmetricSquareRoot( SymmetricMatrix& lhr, 
  const SymmetricMatrix& m )
{
  real eigs[3];
  DenseMatrix e0(3,3), e1(3,3), e2(3,3);
...
AXISBLAS_API void afb::SymmetricExponential( SymmetricMatrix& lhr, 
  const SymmetricMatrix& m )
{
  real eigs[3];
  DenseMatrix e0(3,3), e1(3,3), e2(3,3);
