...
aslp::ParseResult aafc::GeneralSummaryNodeCollectorFactory::Pimpl::TryParseAny( 
...
aafc::CollectorBuildResult aafc::GeneralSummaryNodeCollectorFactory::Pimpl::ParseAndBuildAny( 
  const asli::InputIterator& begin, const asli::InputIterator& end, const ada::NumericalModel& model, 
  aapc::ParseContext& context, SummaryNodeCollectorBuilder& builder )
{
  aafc::CollectorParseResult result = Parser.Parse(begin, end);
  ...
  aaoc::EntityCollector *c = &BuildCollector(collectorType, targetSetName, directionsToCollect, 
                                             result.GetGroupingType(), builder);  

  return aafc::CollectorBuildResult(c, aafc::kGenericCollectorType, parseResult);
}

aaoc::GenericCollector& aafc::GeneralSummaryNodeCollectorFactory::Pimpl::BuildCollector( 
  aafc::CollectorType collectorType, const axis::String& targetSetName, 
  const bool * directionsToCollect, aaocs::SummaryType summaryType, 
  SummaryNodeCollectorBuilder& builder ) const
{
  ...
  else
  {
    ...
    case aafc::kDisplacement:
      c = &builder.BuildDisplacementCollector(targetSetName, summaryType, xState, yState, zState);
      break;
    case aafc::kVelocity:
      c = &builder.BuildVelocityCollector(targetSetName, summaryType, xState, yState, zState);
      break;
    case aafc::kAcceleration:
      c = &builder.BuildAccelerationCollector(targetSetName, summaryType, xState, yState, zState);
      break;
    case aafc::kReactionForce:
      c = &builder.BuildReactionForceCollector(targetSetName, summaryType, xState, yState, zState);
      break;
    case aafc::kExternalLoad:
      c = &builder.BuildExternalLoadCollector(targetSetName, summaryType, xState, yState, zState);
      break;
    default:
...
void aafc::GeneralSummaryNodeCollectorFactory::Pimpl::MarkUndefinedNodeSet( const axis::String& setName, 
