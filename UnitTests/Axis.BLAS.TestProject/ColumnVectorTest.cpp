...
template <>
std::wstring Microsoft::VisualStudio::CppUnitTestFramework::ToString(const ColumnVector& v)
{
...
{		
    ...
    TEST_METHOD_CLEANUP(TearDownTest)
    {
      axis::System::Finalize();
		...
		TEST_METHOD(AttributesTest)
		{
			...
			Assert::AreEqual(1L, m.Columns());
			Assert::AreEqual(5L, m.Length());

			ColumnVector x(4);
			Assert::AreEqual(4L, x.Rows());
			Assert::AreEqual(1L, x.Columns());
		...
		TEST_METHOD(CopyConstructorTest)
		{
			...
			// copy vector
			ColumnVector c(m);
			Assert::AreEqual(4L, c.Rows());
			Assert::AreEqual(1L, c.Columns());
		...
		TEST_METHOD(IncrementColumnVectorTest)
		{
			ColumnVector m(2);
			m.SetElement(0, 1);
			m.SetElement(1, 9);
			...
			// we must ensure that the same ColumnVector is being returned by
			// this statement
		...
		TEST_METHOD(DecrementColumnVectorTest)
		{
			ColumnVector m(2);
			m.SetElement(0, 4);
			m.SetElement(1, 13);
		...
		TEST_METHOD(ScaleMethodTest)
		{
			...
			// we must ensure that the same ColumnVector is being returned by
			// this statement
			Assert::AreSame(m, (ColumnVector&)m.Scale(10));
		...
		{
			ColumnVector v(3), r(3);
			v(0) = 4; v(1) = 5; v(2) = -1;

			r.CopyFrom(v);

			Assert::AreEqual( 4, r(0), REAL_TOLERANCE);
			Assert::AreEqual( 5, r(1), REAL_TOLERANCE);
			Assert::AreEqual(-1, r(2), REAL_TOLERANCE);
		}
    TEST_METHOD(AllocateFromModelMemoryTest)
    {
      axis::foundation::memory::RelativePointer ptr1 = 
        ColumnVector::Create(3);
      axis::foundation::memory::RelativePointer ptr2 = 
        ColumnVector::Create(3);

      ColumnVector& v1 = *(ColumnVector *)*ptr1;
      ColumnVector& v2 = *(ColumnVector *)*ptr2;
    ...
    {
      axis::foundation::memory::RelativePointer ptr1 = 
        ColumnVector::CreateFromGlobalMemory(3);
      axis::foundation::memory::RelativePointer ptr2 = 
        ColumnVector::CreateFromGlobalMemory(3);
